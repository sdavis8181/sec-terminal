...
# Set your SEC EDGAR identity (required by SEC)
...
with st.sidebar:
  ...
  lookback_quarters = st.slider(
      "Historical Lookback (Quarters)", min_value=12, max_value=24, value=16, step=4
  )
...
def fetch_and_parse_ticker(ticker, num_quarters):
  ...
  for f in filings_10q:
    ...
    rev = parse_multi_val(inc_df, ["Revenue"])
...
try:
  ...
  ax4.set_xticks(x)
  ax4.set_xticklabels(df_raw["Period"], rotation=45, ha="right", fontsize=7)
