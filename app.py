...
def calculate_fcf_safe(df):
  try:
    ...
    df_cf_raw["OCF"] = pd.to_numeric(df_cf_raw["OCF"], errors="coerce").fillna(0)
    df_cf_raw["Capex"] = pd.to_numeric(
        df_cf_raw["Capex"], errors="coerce"
    ).fillna(0)
    ...
    for idx, row in df_cf_raw.iterrows():
      p_date = pd.to_datetime(row["Period"], errors="coerce")
      curr_year = p_date.year if pd.notna(p_date) else 0
